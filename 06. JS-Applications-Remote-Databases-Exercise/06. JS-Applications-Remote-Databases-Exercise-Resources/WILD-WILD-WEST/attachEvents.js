function attachEvents() {
  const baseUrl = 'https://testapp-c8731.firebaseio.com/players/';
  ...
  const playerTemplate = (player) => {
    ...
    const savePlayer = async () => {
      await remote(httpMethod.PUT, baseUrl + player._id+'.json', JSON.stringify(player))
        .catch(error);
    ...
    const deletePlayer = () => {
      remote(httpMethod.DELETE, `${baseUrl}${player._id}.json`)
        .then($(html).remove())
        .catch(error);
    };
